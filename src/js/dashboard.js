$(document).ready(function() {
  ...
  //     // const countryNews = `https://newsapi.org/v2/everything?${news_key}&${news_keyword}${news_country}&${news_language}&${news_sort}`;
  //     axios
  ...
  $("#ddChartCountries").on("change", function() {
    ...
    $("input:radio[name=timeframe]")
      .filter("[value=month]")
      .prop("checked", true);
    $("input:radio[name=caseTypeRadio]")
      .filter("[value=confirmed]")
      .prop("checked", true);
    $("input:radio[name=chartTypeRadio]")
      .filter("[value=bar]")
      .prop("checked", true);
...
function continentsDoughnut() {
  return axios
    .get(
      "https://corona.lmao.ninja/v2/continents?yesterday=true&sort=cases&allowNull=true"
    )
    .then((response) => {
      let charts = [];
      let continentsLabel = [];
...
function getNewsResults(data) {
  ...
  let output = "";
  newsResultsNumber.innerText = `Showing ${data.articles.length}`;
      ...
      columns: [
        {
          data: "name",
          title:
            'Country <small class="text-dark font-weight-600">(Population)</small>',
