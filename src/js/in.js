  ...
  data.statewise.forEach(function(states) {
    if (states.statecode === "TT") {
      document.getElementById("last-updated").innerHTML =
        'Last updated <span class="text-gray-800">' +
        states.lastupdatedtime +
        "</span>";
      document.getElementById("number-active").innerText = parseInt(
        states.active
      ).toLocaleString();
      document.getElementById("number-confirmed").innerText = parseInt(
        states.confirmed
      ).toLocaleString();
      document.getElementById("number-recovered").innerText = parseInt(
        states.recovered
      ).toLocaleString();
      document.getElementById("number-deaths").innerText = parseInt(
        states.deaths
      ).toLocaleString();
...
function countryDataSet(data) {
  ...
  $("#icmrReport").html(
    ...
    ).toLocaleString()}</span> samples have been tested as of ${
      lastArrTested.updatetimestamp
    }. View official report by ICMR <a href="${
      lastArrTested.source
    }" target="_blank">here</a>`
  );
  data.cases_time_series
    .slice(Math.max(data.cases_time_series.length - 30, 1))
  ...
  //Get 14 days
  data.cases_time_series
    .slice(Math.max(data.cases_time_series.length - 14, 1))
    .forEach(function(daily) {
...
function fillNewsCards() {
  axios.get(inNews).then((response) => {
    ...
    data.articles.forEach(function(item) {
      ...
      if (item.title && item.content) {
        output += `
            ...
            item.source.name
          } ${timeDifference(item.publishedAt)}</p>
